...
@Path("/Test_send_result")
public class Test_Post_Campaign2 {
	static String IV = "AAAAAAAAAAAAAAAA";
	static String encryptionKey = "0123456789abcdef";
	static long minInterval = 30;
    ...
    static int point_AudioSensor 		    = 3;
    ...
    static String initialSubmissionStatus = "stored";
	...
	public Response respond( String input, @QueryParam("paddingSize") String paddingSize,
		...
		CampaignExtended campaign_obj = null;
		...
		{
			campaign_obj = gson.fromJson(arrayAllCampaign.get(0), CampaignExtended.class);
		...
		if (submittedTime.size() == 0) //Participant submit for the first time, always accept
		{
			...
			insertSubmission(gson, mongo, userID, CampaignID,result, submissionLat, submissionLon, campaign_obj);
			/*
	...
	public void processPayment(ArrayList<IncentiveType> incentiveList, String userID, MongoClient mongo) throws IOException
	{
		...
		{			
			...
			{
				System.out.println("This incentive is TIME_DEPENDENT FLAT PAYMENT");
			}
		}
	...
	public void addPayment (String userID, MongoClient mongo, int amount) throws IOException
	{
		...
		while (cursor.hasNext()) {
		    BasicDBObject obj = (BasicDBObject) cursor.next();
		    currentMoney = obj.getInt("money");	
		    currentFlatPaymentMoney = obj.getInt("moneyFromFlatPayment");
		}
		...
		{
			int reducedAmount = flatPaymentLimit - currentFlatPaymentMoney;
	...
	{	
		...
		for(int i=0;i<ansArray.length;i++)
		{
			...
			switch (quesType)
			{
				case "FreeTextSingle":       		point += (point_FreeTextSingle * coefficient); 		  break;
				case "FreeTextMulti" :  			point += point_FreeTextMulti * coefficient;  		  break;
				case "MultipleChoiceSingle":		point += point_MultipleChoiceSingle * coefficient;    break;
				case "MultipleChoiceMulti":			point += point_MultipleChoiceMulti * coefficient;	  break;
				case "ContRange":					point += point_ContRange * coefficient;			  	  break;
				case "AudioSensor":					point += point_AudioSensor * coefficient;			  break;			
				case "FreeNumericSingle":			point += point_FreeNumericSingle * coefficient;	  	  break;
				case "UploadPhoto":					point += point_UploadPhoto * coefficient;			  break;
				case "DateInput":					point += point_DateInput * coefficient;			  break;
				case "TimeInput":					point += point_TimeInput * coefficient;			  break;
				case "WifiSensor":					point += point_WifiSensor * coefficient;			  break;
			}		
		}
	...
	public static void addInitialSubmissionStatus(MongoClient mongo, String campaignID, String timestamp, String userID)
	{
		List<ArrayList> updatedList = new ArrayList();
		...
		while (cursor.hasNext())
		{
		    ...
		    BasicDBList list = (BasicDBList) user.get("trackedSubmission");
		    for (int e=0;e<list.size();e++)
			{
		    	//System.out.println(list.get(e));
		    	List temp = (List) list.get(e);
		    	updatedList.add((ArrayList) temp);	    	
			}	
		}
		
		ArrayList newSubmissionStatus = new ArrayList(){};
	    newSubmissionStatus.add(campaignID);
	    newSubmissionStatus.add(timestamp);
	    newSubmissionStatus.add(initialSubmissionStatus);
	    
	    updatedList.add( newSubmissionStatus );
	    
	    //System.out.println("New list is " + updatedList.toString());
	    
	    BasicDBObject newDocument = new BasicDBObject();
	    newDocument.put("trackedSubmission", updatedList);
	...
	public static void insertSubmission(Gson gson, MongoClient mongo, String userID, String ID_of_campaign,String submissionContent, String lat, String lon, CampaignExtended cam_obj) throws IOException
	{
		...
		String localVar = new DateTime().toString();
		System.out.println("DAteTime value is " + localVar);
		document.put("submissionTime", localVar);
		document.put("submissionQuality","good");
		if (cam_obj.getTrackedSubmissionValue() == true) document.put("status",initialSubmissionStatus);
		table.insert(document);
		
		if (cam_obj.getTrackedSubmissionValue() == true)
		{
			addInitialSubmissionStatus(mongo, ID_of_campaign, localVar, userID);
		}
